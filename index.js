...
function convertUnit() {
    ...
    resultLength.textContent = `${value} meters = ${numFeet.toFixed(3)} feet | ${value} feet = ${numMeter.toFixed(3)} meters`
   
   const numLiters = value * 0.264;
   const numGallons = value * 3.78541;
   resultVolume.textContent = `${value} liters = ${numLiters.toFixed(3)} gallons | ${value} gallons = ${numGallons.toFixed(3)}`
   
   const numKg = value * 2.204;
   const numPounds = value * 0.453592;
   resultMass.textContent = `${value} kilos = ${numKg.toFixed(3)} pounds | ${value} pounds = ${numPounds.toFixed(3)}`
}
